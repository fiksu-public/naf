    ...
    class Machine < Base
      ...
      def retrieve_old_logs
        ...
        jsons.reverse_each do |elem|
          output.insert(0, "&nbsp;&nbsp;<span>#{elem['output_time']} <font color='333399'><b>jid(#{elem['job_id']}):</b></font> #{elem['message']}</br></span>")
        end
      ...
      def parse_files
        ...
        files.each do |file|
          if logs_size > LOG_SIZE_CHUNKS
            ...
            job_id, line_number = info[1..-2].split('/')
            if limit.present? && limit != line_number
              @stopped_at_id = job_id
        ...
        if last_id_checked.present? && last_line_checked.present?
          if last_id_checked == -1
            ...
            files.each_with_index do |file, index|
              # Retrieve the job_id and line_number associated with the log
              info = file.scan(/\/\d+\/\d+_/).first
              job_id, line_number = info[1..-2].split('/')

              # Compare the information from the log file with the information sent in params
              if job_id.to_i == last_id_checked && line_number.to_i <= last_line_checked
      ...
      def find_new_files_not_read
        ...
        if newest_id_checked.present? && newest_line_checked.present?
          files.each_with_index do |file, index|
            ...
            if job_id.to_i == newest_id_checked && line_number.to_i <= newest_line_checked
              files = files[0..index]
              break
            end
          end
