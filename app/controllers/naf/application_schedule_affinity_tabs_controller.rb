module Naf
  class ApplicationScheduleAffinityTabsController < Naf::ApplicationController
    ...
    def create
      ...
      if @tab.save
        redirect_to(naf.application_application_schedule_application_schedule_affinity_tab_path(@application, @application_schedule, @tab), :notice => "Application Schedule Affinity Tab '#{@tab.affinity_name}' was successfully created.")
      else
    ...
    def update
      ...
      if @tab.update_attributes(params[:application_schedule_affinity_tab])
        redirect_to(naf.application_application_schedule_application_schedule_affinity_tab_path(@application, @application_schedule, @tab), :notice => "Application Schedule Affinity Tab '#{@tab.affinity_name}' was successfully updated.")
      else
