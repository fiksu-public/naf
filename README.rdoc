...
5. Now import these new gems
    
    bundle install

6. Run the generator to bring in the necessary configurations, initializers, and migrations into your Rails app.
...
7. This has created/edited the following files:
    ...
    config/routes.rb  (This file is edited to mount Naf as a rails engine, at the default location "/job_system")

8. All you have to do is to run this new migration, and you have your database all set up.

    rake db:migrate

9. Try running the Rails console, and see that you have the Naf models loaded.

    rails console
    Naf::Affinity.all

10.  If you use a base controller for your application's authentication, edit the line from config/initializers/job_system_schema_initializer.rb

    CONTROLLER_NAME_STR = "::ApplicationController"

11. Add the Naf javascript files to the asset pipeline.  In app/assets/javascripts/application.js, insert:
    
    //= require naf

12. Add the Naf stylesheet files to the asset pipeline.  In app/assets/stylesheets/application.css, insert:

     *= require naf

13.  Now the Naf Job System engine should be mounted at '/job_system'
