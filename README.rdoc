...
2. Set up partitioning
   script/rails runner Process::Naf::Janitor.run --create-infrastructure ::Naf::Job,::Naf::JobIdCreatedAt,::Naf::JobAffinityTab
...
3. For each server to be configured:

3a. Run:
    gem install remote_syslog

3b. Copy the following to /etc/log_files.yml
...
3c. Set up log rotation for the things that matter
...
3.  Clone the structure of the naf_development database to the naf_test database

      rake db:test:clone_structure

4.  Now change back to the root directory of the engine repository, and run:
