...
4. By default the Naf tables live on your primary database.  You can optionally configure the naf tables to live on a specific database connection, add entries: naf_development, naf_production etc. to your config/database.yml.  For example, if you had a database named: sparedb_development, you can have the Naf tables live there by adding:
...
5. Now bring in all of the Naf schema migrations.  

    rake naf:install:migrations

6. This has copied all the Naf migrations to a new folder db/naf/migrate folder, run the schema changes.  When you bundle the latest version of Naf for production, make sure to add this command to your deploy script, so that you can change to the Engine's latest schema.

    rake naf:db:migrate

7. Try running the Rails console, and see that you have the Naf models loaded, and mapped to the database.

    rails console
    Naf::Affinity.all

8. If you use a base controller for your application's authentication, edit the line from config/initializers/job_system_initializer.rb

    CONTROLLER_NAME_STR = "::ApplicationController"

9. Now you can visit the Naf Job System UI mounted at the default path '/job_system' (Configurable in config/routes.rb)

10. You can change the name shown on the page, edit the 'main_app_title' attribute in config/job_system_config.yml.

11. Undo all of this setup by running the following rake task, rolling back all the naf migrations, and deleting all copied/installed files.  WARNING: This will wipe away all of your data in your Naf tables.
