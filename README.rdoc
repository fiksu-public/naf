...
2. Run the generator to bring in the necessary configurations, initializers, and migrations into your Rails app.
...
5. Try running the Rails console, and see that you have the Naf models loaded.
...
7. Add the Naf javascript files to the asset pipeline.  In app/assets/javascripts/application.js, insert:
    
    //= require naf

8. Add the Naf stylesheet files to the asset pipeline.  In app/assets/stylesheets/application.css, insert:

    *= require naf

9. Now the Naf Job System engine should be mounted at the default path '/job_system' (change this in config/routes.rb)
